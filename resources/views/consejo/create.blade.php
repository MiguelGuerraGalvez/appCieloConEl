...
<body class="bg-[#EBEBEB] flex justify-center md:pt-8">
    ...
    <div class="w-[80vw] mt-8">
        <section class="mt-8 mb-8 flex flex-col items-center gap-6">
            <h1 class="text-4xl text-center font-bold">ADMINISTRACIÓN DE ITINERARIOS</h1>
            
            <div class="grid items-stretch grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($itinerariosNoAceptados as $itinerario)
                    <article class="bg-white rounded-md p-4 flex flex-col h-full">
                        <h4>{{ $itinerario->dia }}</h4>
                        <h5>Nazarenos</h5>
                        <p>{{ $itinerario->nazarenos }}</p>
                        <p>{{ $itinerario->recorrido }}</p>

                        <form class="flex flex-col flex-1 h-full" action="{{ route('consejo.aceptarItinerario') }}" method="post">
                            @csrf
        ...
        <section>
            <h1 class="text-4xl text-center font-bold">ADMINISTRACIÓN DE NUEVAS HERMANDADES</h2>
        ...
        <section>
            <h1 class="text-4xl text-center font-bold">ADMINISTRACIÓN DE CARTELES</h1>
        ...
        <section>
            <h1 class="text-4xl text-center font-bold">ADMINISTRACIÓN DE PREGONES</h1>
